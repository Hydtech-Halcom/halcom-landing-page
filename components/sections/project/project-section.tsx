...
const groupedProjectDefinitions: Array<
  Omit<ProjectGalleryGroup, "projects"> & { source: ProjectCardData[] }
> = [
  {
            ...
            {group.projects.map((project, index) => (
              <CarouselItem key={project.id} className="pl-0">
                <button
                  ...
                  onClick={() => onOpenGallery(group, index)}
                >
                  <Image
                    src={project.imgSrc}
                    alt={project.imgAlt}
      ...
      <DialogContent className="max-w-none sm:max-w-none w-[95vw] h-[92vh] p-0 overflow-hidden flex flex-col gap-0">
        ...
        <div className="flex-1 min-h-0 px-4 pb-4">
          <Carousel
            ...
            opts={{ loop: true, startIndex: selectedGallery.initialIndex }}
            className="h-full"
          >
...
export function ProjectSection({
  ...
  const [selectedGallery, setSelectedGallery] =
    useState<ProjectGalleryState | null>(null);

  const { regularProjects, groupedProjects } = useMemo(() => {
    const visibleProjectIds = new Set(projects.map((project) => project.id));

    const availableGroupedProjects: ProjectGalleryGroup[] =
      groupedProjectDefinitions
        .map((definition) => ({
          id: definition.id,
          title: definition.title,
          description: definition.description,
          client: definition.client,
          projects: definition.source.filter((project) =>
    ...
    const groupedIds = new Set(
      availableGroupedProjects.flatMap((group) =>
